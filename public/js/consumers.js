...
// Đồng bộ giỏ hàng với backend (Docker)
function syncCartItemBackend(package_id, selected_type, quantity) {
    fetch("/backend/data-consumer-service/api/cart.php", {
        method: "POST",
        credentials: 'include',
...
async function loadCartFromServer() {
    try {
        const res = await fetch("/backend/data-consumer-service/api/cart.php?action=get", {
            credentials: 'include'
        });
...
function removeFromCart(cartId) {
    ...
    if (item) {
        fetch("/backend/data-consumer-service/api/cart.php?action=remove", {
            method: "POST",
            credentials: 'include',
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                package_id: item.id,
                selected_type: item.selectedType
            })
        });
    }
...
async function checkoutCart() {
    ...
    try {
        // Gọi qua router backend (payment)
        const res = await fetch("/backend/data-consumer-service/index.php?page=payment&action=create", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                user_id: userId,
...
async function accessDataset(datasetId) {
    try {
        const res = await fetch(`/backend/data-consumer-service/api/data_access.php?dataset_id=${datasetId}`, {
            credentials: 'include'
        });
        const data = await res.json();
        if (data.success) {
